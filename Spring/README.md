...
웹에 존재하는 모든 자원에 고유한 URL를 부여해 활용하는 것, 자원을 정의하고 자원에 대한 주소를 지정하는 방법론

- 자원(URL): 모든 자원은 고유한 ID를 가지고 ID는 서버에 존재하고 클라이언트는 각 상태를 조작하기 위해 요청을 보낸다.
  - Http에서 이러한 지원을 구별하는 ID는 'Students/1'같은 HTTP URL
- 행위(Method): 클라이언트는 URL를 이용해 자원을 지정하고 조작하여 사용
  - Http 프로토콜에서는 Get, Post, Put, Delete같은 Method를 제공한다.
- 표현: 클라이언트가 서버로 요청을 보냈을 때 서버가 응답하는 자원의 상태
  - Rest에서 하나의 자원은 Json, Xml, Text, Rss등 여러 형태로 표현할 수 있다.

### 특징

1. 클라이언트, 서버 구조 - 자원이 있는 Server, 요청하는 Clinet
2. 무상태 - Http는 무상태 프로토콜이므로 각각 독립적으로 사용한다.
3. 캐시처리가능 - 웹 표준을 그대로 사용하므로, 웹에서 사용하는 기존의 인프라 그대로 활용가능
4. 계층화 - API서버는 순수 비즈니스 로직을 수행하고 그 앞단에 사용자 인증, 암호화, 로드 밸런싱등을 하는 계층을 추가하여 구조상 유연성을 줄 수 있다.
5. 인터페이스 일관성 - URL로 지정한 지원에 대한 조작을 통일하고 한정적인 인터페이스로 수행한다. HTTP 표준만 따르면 모든 플랫폼에서 사용 가능하다.
6. 자체 표현 도구 - 메소드 + URL로 이루어져 있어 어떤 메서드에 무슨 행위를 하는지 알 수 있으며 REST API 자체가 매우 쉬어서 API 메시지 보고도 API를 이해할 수 있다.

### 장점

1. 쉬운 사용 - Http 프로토콜 인프라 그대로 사용하므로 따로 인프라 만들 필요가 없다.
2. 클라이언트 - 서버 역할의 명학환 분리
   - 클라이언트 REST API를 통해서 정보를 주고 받고, 무상태 특징에 따라 클라이언트의 Context를 유지할 필요는 없다.
3. 특정 데이터를 표현을 사용가능 - REST API는 헤더 부분에 URL 처리 메소드를 명시하고 필요한 실제 데이터를 body에 표현할 수 있도록 분리시켰다.

### 단점

1. 메서드의 한계 - Http 메소드를 이용하여 URL 표햔한다.
   - 쉬운 사용이 가능하다는 장점이 있지만 반대로 메소드 형태가 제한적인 단점이 있다.
2. 표준이 없다.
   - 설계 가이드일 뿐이지, 명확한 표준이 없다.

### RESTFul API

- Rest를 기반으로 API를 웹으로 구현한 것
