...
클라이둥 컴퓨팅: 서로 다른 물리적인 위치에 존재하는 컴퓨터 리소스를 가상화 기술로 통합해 제공하는 기술

- 클라우드에 저장하고 웹 브라우저에 필요한 어플리케이션을 구동해 원하는 작업을 수행할 수 있는 사용자 중심의 컴퓨터 환경

- 분류 조건: 얼마만큼 사용자가 관리하고 얼마만큼 클라우드에서 제공받는가
...
- OS, 미들웨어 등은 사용자가 직접 설치 관리한다.
  - 즉, 기업이 준비 해 놓은 환경에서 우리가 직접 작업 한다.
...
- Node.js와 Java와 같은 런타임을 미리 깔아놓고, 거기에 소스코드를 넣어서 돌리는 구조
  - 소스 코드만 적어서 빌드를 하고 컴파일은 클라우드에서 결과만 가져오는 것

SaaS(Software as a Service): 서비스 제공자가 서버 등의 기본 인프라는 물론 바로 사용할 수 있는 애플리케이션 최종 사용자에게 제공하는 서비스

- 클라우드를 통해 제공되는 소프트웨어로 별도의 설치나 전환 과정 없이 퍼블릭 클라우드에 설치되어 있는 애플리케이션
- 서비스를 인터넷을 통해 제공받는 것
  - 반드시 인테넛을 접속해야만 사용이 가능하다
...
- 데이터 베이스를 보호하기 위함이다
- 소프트웨어 요청이 매우 커지게 되면 DB가 뻗을 수 있다
...
수집 환경과 수집 데이터 처리하고자 하는 시스템을 고려하여 수집 기술을 선택해야 한다.

1. 확장성: 수집의 대상이 되는 시스템이 얼마나 늘어날 것인가
2. 안정성: 수집되는 데이터가 손실되지 않고 안정적으로 저장 가능한가
3. 유연성: 다양한 데이터의 형식과 접속 프로토콜을 지원 하는가
4. 주기성: 수집 데이터가 실시간으로 반영되야 하는 가 혹은 배치 처리를 해도 되는가

## API Gateway

- 기본적으로 API 서비스는 원격 요청을 수락하고 응답을

백엔드 서비스 또는 데이터와 접속하고 API 호출에 대한 정책, 인증 및 일반 엑세스 제어를 적용하여 중요한 데이터를 보호하는 트래픽 관리자

즉, API 서버 앞단에서 모든 API 서버들의 end point를 단일화로 묶어주고 API에 대한 인증과 인가 기능에서 부터 메세지에 따라서 여러 서버로 라우팅하는 고급기능까지 많은 기능을 담당할 수 있다.

- 백엔드 시스템 및 서비스에 대한 엑세스를 제어하는 방법
- 외부 클라이언트와 백엔드 서비스 간의 통신을 최적화 하여 클라이언트에게 원할한 경험을 제공하도록 설계
- 요청을 적절한 서비스로 라우팅하고 요청자에게 응답을 다시 보내는 기능을 담당한다.
- 데이터와 API간의 보안 연결을 유지하고 회사 내부 및 외부에서 로드 밸런싱을 포함한 API 트래픽 및 요청을 적용한다.
- 클라이언트의 모든 API 호출을 가져와 요청 라우팅, 구성 및 프로토콜 변환을 사용하여 올바른 마이크로서비스로 라우팅한다.

1. 인증/인가 관련: API를 호출하는 클라이언트에 대한 Identity를 확인해 주는 기능, 클라이언트가 API를 호출 할 수 있는 권한이 있는지 확인해 주는 기능
2. API 토큰 발급: 매번 사용자의 인가/인증 절차가 복잡하기 때문에 사용자 인가가 끝나면 토큰을 통해서 identity를 확인한다.

- 1. API 클라이언트 -> API Gateway: 클라이언트 인증 정보
- 2. API Gateway -> 인증 서버: 인증 요청
- 3. 인증 서버 -> API Gateway: 인증 성공
- 4. API Gateway -> API 클라이언트: apitoken

3. Endpoint 별 API 호출 인증: 각 APIToken을 이용하여 API Gateway는 API 호출을 승인할지 여부 결정한다, 토큰 저장소로 부터 읽어와서 비교한 후 API 호출 가능 여부를 결정한다.
4. Endpoint 별 API 요청 인가: API의 대한 권한을 줄 수 있는 것, 직접 권한을 토큰에 연결하는 것이 아니라 역할으라는 개념을 두고 역할별로 권한을 연결하는 개념
5. API 라우팅: 같은 API라도 사용하는 서비스나 클라이언트에 따라서 다른 endpoint를 이용해서 서비스를 제공
6. 백엔드 API 서버로의 로드 밸런싱: API 게이트 웨이 뒷단에 다수의 API 서버가 있을 때 여러개의 서버로 부하를 분산하는 기능
7. 서비스 및 클라이언트 별 엔드포인트 제공: 같은 API를 여러개의 엔드포인트를 통해서 서비스를 제공할 수 있다.

## AB Test

Apache Bench 으로서 웹 서버의 성능 테스트를 위해서 접속을 강제로 늘려서 부하를 주는 스트레스 테스트
...
스토리지에 데이터를 저장하기 전에 Journal 영역에 데이터의 변경 이력을 저장하고 스토리지에 데이터 변경 내역을 저장하는 활동

### 목적

시스템 장애 등 스토리지에 장애가 발생하였을 경우, 스토리지의 장애 복구 과정을 빠른 시간에 원활하게 처리할 수 있도록 하기 위함

### 단점

System Call 수준에서만 일관성을 유지하기 때문에 write를 여러번 사용하는 작업의 중간에 장애가 발생하는 경우 데이터의 복사본이 원본보다 작게 만들어 질 수 있다.

### 원리

파일, 디렉터리에 변경을 반영하기 위한 파일 시스템 수정은 여러 개의 쓰기 오퍼레이션으로 구분

- 쓰기 동작 중 전원 공급 문제나 시스템 오류와 같은 인터럽트가 발생하여 특정 데이터를 비정상적인 중간 상태로 남겨둘 수 있다.

## Journaling File System

파일 시스템에 변경사항을 반영하기 전에 저널(파일 시스템의 지정된 영역 안의 원형로그)안에 생성되는 변경사항을 추적하는 파일 시스템

- 시스템 충돌이나 전원 문제가 발생하면, 더 빠르게 online 상태로 돌아온다.
