...
- 통신 매체가 되는 통신 프로토콜이 있고 통합하면의미를 전달하느 통신 프로토콜이 있다
...
- 애플리케이션 프로토콜은 사용자 공간을 처리 - ?????
  - 클라이언트 프로세스와 httpd 프로세스를 통해서 애플리케이션 계층 프로토콜인 HTTP 요청이 통신하는 모습을 보여준다
  - 소캣: 요청을 보내는 애플리케이션이 구멍에 데이터를 넣는다
