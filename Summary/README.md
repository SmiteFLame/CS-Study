...
WAS가 실행되면서 DB와 미리 Connection을 해놓은 객체들을 Pool에 저장해 두었다가 클라이언트 요청이 오면 Connection을 빌려주고 다시 Connetion을 반납하는 방식
...
- 블록은 동기화랑 무관한 이야기 이다.
- 메소드 호출한 후로 시간이 오래 걸리면 블로킹 시스템이다

### 동기 비동기 블록 논블록 조합

- 동기 + 블럭: 모든 일들이 순차적으로 일어 나게된다
- 비동기 + 블럭: 중간에 블럭되는 지점을 제외한 이전/이휴는 각자 자신의 일을 한다.
- 동기 + 논블록: 내가 특정 일을 하는 경우 다른 일은 쉬는 경우의 수가 생긴다
- 비동기 + 논블록: 병목현상이 매우 발생활 확률이 높다
  - 복잡한 곳에서는 멀티 스레드를 통해서 성능을 올릴 수 있다.
...
- Spring뿐 만 아니라 제 3의 라이브러리 까지 기본적으로 제공해준다.
...
- 서비스의 지속성을 보장하기 위해서 둔다. 다른 장비가 다운되면 반대쪽 장비에서 서비스를 하기 위함이다

- FailOver: 평소에 사용하는 서버와 그 서버의 클론 서버를 가지고 있다가 사용 서버가 장애로 사용이 어렵게 되었을 경우 클론 서버로 그 일을 대신 처리 해주는 것
  - 즉, 무정시 시스템을 구축하는 것
- LoadBalancing: 부하 분산
...
2. Stateless(무상태성): 작업을 위한 상태 정보를 따로 저장하지 않고 관리하지 않는다
