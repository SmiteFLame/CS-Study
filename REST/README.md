...
|           | GET                                                            | POST                                       |
| --------- | -------------------------------------------------------------- | ------------------------------------------ |
| 요청      | 모든 필요한 데이터를 URL에 포함하여 요청한다.                  | 추가적인 데이터를 body에 포함할 수 있다.   |
| Form data | Actiom URL에 Query String parameter로 전달                     | HTTP Request의 message body에 나타난다     |
| history   | 파라미터들은 URL 일부분이기 떄문에 브라우저 히스토리에 남는다. | 히스토리에 저장되지 않는다.                |
| 길이제한  | URL이여서 2048 혹은 서버에 따라 다르다                         | 제한 없다                                  |
| 가시성    | 메소드는 모두에게 보여진다.                                    | URL이 보이지 않는다                        |
| 캐시      | 같은 요청을 보내도 항상 같은 응답                              | 같은 요청을 보내도 다른 응답이 올수도 있다 |
...
- API에서 리소스에 대해 어떠한 행동을 할 수 있는지 URL을 전달하여 클라리언트가 참고하고 사용할 수 있도록 한다
- 리소스의 상태에 따라 링크 정보가 바뀌며 동적으로 리소스를 구성
...
- PUT를 요청할 때에는 데이터 전체를 교체하기 때문에 입력값이 전부 들어오게 된다.
