...
- 자바가 .java를 생성하면 javac(자바컴파일러)가 컴파일하여 클래스를 바이트 코드로 변환해준다.

2. Execution Engine

- Class Loader에 의해 메모리에 적재된 클래스들을 기꼐어로 번역하여 명령어 단위로 실행하는 것

3. Garbarge Collector

- 메모리 영역에 생성된 객체들 중 참조되니 잫은 객체들을 제거하는 역할

4. Runtime Data Area

- JVM의 메모리 영역으로 자바 애플리케이션 실행시 데이터를 적재하는 영역
...
- 형태가 같은데 다른 기능을 하는 것을 의미한다
  ...
  2. 오버라디이 - 부모 클래스의 메소드를 자식 클래스의 용도에 맞게 재정의하는 것
