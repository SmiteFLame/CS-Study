...
인터페이스가 default키워드로 선언되면 메소드를 구현 할 수 있다.

- 이를 구현하는 클래스는 default 메서드를 오버라이딩 할 수 있다.
- 만약 인터페이스가 면경이 되면, 인터페이스를 구현하는 모든 클래스들이 해당 메서드를 구현해야 하는 문제를 방지한다.
- 인터페이스에 static 메서드를 선언함으로써 간단한 기능을 가지는 유틸리성 인터페이스를 만들 수도 있다.
