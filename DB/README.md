...
데이터 베이스에서 데이터들이 정확성, 일관성 , 유효성을 유지하는 것을 말합니다.
...
특정 데이터를 찾을 때 조인이 많이 되는 경우의 수가 있습니다. 그러한 데이터를 자주 찾으려고 할때 계속 조인이 된다고 하면 조인이되는 시간이 걸리고 오래 걸리게 됩니다. 그러한 경우 인덱스를 활용할 수 있습니다. 인덱스를 활용하면 미리 조인이 된 데이터를 가지고 있어서 빠르게 데이터를 찾고 불러올 수 있어서 더 조인을 하는 시간을 줄일 수 있습니다. 이렇게 데이터의 양이 많고 변경보다 검색이 더 많은 경우, 인덱스로 사용하고자 하는 필드 값이 다양한 경우 테이블에 대한 동작 속도를 높여줄 수 있습니다.
...
- NoSQL
  - RDMS가 비대해짐에 따라 관계가 복잡해져, 이를 극복하기 위해 등장하게 된 데이터 베이스
